    ...
    <div>
      <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-gray-500">{pagination?.total ?? 0} actas</p>
        </div>
        {canEdit && (
        ...
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-gray-100">
            {loading ? (
              ...
              <tr><td colSpan={6} className="px-4 py-8 text-center text-gray-400">No hay actas registradas.</td></tr>
            ) : (
              items.map((a) => (
                <tr key={a.id} className="hover:bg-brand-50/40">
                  <td className="px-4 py-3">
                    <Link href={`/panel/actas/detalle?id=${a.id}`} className="font-medium text-brand-700 hover:underline">{a.actaNumber}</Link>
                  </td>
                  <td className="px-4 py-3 text-gray-700">{a.title ?? "—"}</td>
                  <td className="px-4 py-3 text-gray-600">{a.actaDate.slice(0, 10)}</td>
                  <td className="px-4 py-3 text-gray-600">{a.folios ?? "—"}</td>
                  <td className="px-4 py-3 text-gray-600">{a.rowCount || "—"}</td>
                  <td className="px-4 py-3 text-right">
                    <div className="flex items-center justify-end gap-3">
                      <a href={`${apiUrl}/api/actas/${a.id}/pdf`} target="_blank" rel="noopener noreferrer" className="text-xs text-brand-600 hover:underline">PDF</a>
                      {canEdit && (
                        <button onClick={() => void remove(a.id)} className="text-xs text-red-600 hover:underline">Eliminar</button>
                      )}
                    </div>
                  </td>
                </tr>
      ...
      {pagination && pagination.totalPages > 1 && (
        <div className="mt-4 flex items-center justify-between text-sm text-gray-600">
          ...
          <div className="flex gap-2">
            <button disabled={page <= 1} onClick={() => setPage((p) => p - 1)} className="rounded-lg border border-gray-300 px-3 py-1.5 disabled:opacity-50">Anterior</button>
            <button disabled={page >= pagination.totalPages} onClick={() => setPage((p) => p + 1)} className="rounded-lg border border-gray-300 px-3 py-1.5 disabled:opacity-50">Siguiente</button>
          </div>
        </div>
      )}
    </div>
